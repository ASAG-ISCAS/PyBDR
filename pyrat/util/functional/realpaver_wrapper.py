...
import subprocess
import re
from pyrat.util.functional.auxiliary import get_system
...
class Variable:
    def __init__(self, var_name, lower_bound, upper_bound, lower_bracket, upper_bracket):
...
class RealPaver:
    ...
    def _build(self):
        ...
        self.__input += 'precision = ' + "{:.20e}".format(self._search_precision) + ',\n'
        self.__input += 'mode = ' + self._search_mode + ',\n'
        self.__input += 'number = '
        self.__input += str(self._search_number) if isinstance(self._search_number, int) else self._search_number
        self.__input += ' ;'
        ...
        if len(self.__constants) > 0:
            ...
            input_constants = ', \n'.join([" " + c.to_input() for c in self.__constants])
    ...
    def add_variable(self, name, lower_bound, upper_bound, lower_bracket, upper_bracket):
        self.__variables.append(Variable(name, lower_bound, upper_bound, lower_bracket, upper_bracket))
    ...
    def set_branch(self, choice='lf', parts=2, precision=1e-8, mode='paving', number=1024):

...
class IntervalOld(Geometry.Base):
    ...
    def __contains__(self, item):
        ...
        def __contains_interval(other: IntervalOld):
    ...
    def __add__(self, other):
        if isinstance(other, IntervalOld):
            ...
            return IntervalOld(self.inf + other.inf, self.sup + other.sup)
        elif isinstance(other, (Real, np.ndarray)):
            return IntervalOld(self.inf + other, self.sup + other)
        else:
            raise NotImplementedError
    ...
    def __getitem__(self, item):
        return IntervalOld(self.inf[item], self.sup[item])

    def __setitem__(self, key, value):
        if isinstance(value, IntervalOld):
    ...
    def __matmul__(self, other):
        ...
        def __matmul_interval(rhs: IntervalOld):
            b = self * rhs
    ...
    def __rmatmul__(self, other):
        def __rmatmul_array(lhs: np.ndarray):
            from pyrat.deprecated.interval_matrix import IntervalMatrix
    ...
    def __mul__(self, other):
        def __mul_interval(rhs: IntervalOld):
            bd = np.vstack(
                [
                    self.inf * rhs.inf,
                    self.inf * rhs.sup,
    ...
    def __neg__(self):
        return IntervalOld(-self.sup, -self.inf)
    ...
    def __sub__(self, other):
        if isinstance(other, (Real, np.ndarray)):
            ...
            if other.type == Geometry.TYPE.INTERVAL:
                return IntervalOld(self.inf - other.sup, self.sup - other.inf)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
    ...
    def proj(self, dims):
        return IntervalOld(self.inf[dims], self.sup[dims])
    ...
    def boundary(self, max_dist: float, element: Geometry.TYPE):
        def __boundary_interval():
            ...
            for i in range(self.dim):
                ...
                bd.append(
                    [IntervalOld(cur_data[:, 0], cur_data[:, 1]) for cur_data in data]
                )
    ...
    def split(self, dim: int):
        ...
        inf[dim] = c
        sup[dim] = c
        return IntervalOld(self.inf, sup), IntervalOld(inf, self.sup)
    ...
    def rand(dim: int):
        ...
        return IntervalOld(bd[:, 0], bd[:, 1])

    @staticmethod
    def zero(dim: int):
        return IntervalOld(np.zeros(dim), np.zeros(dim))

...
from .continuous_system_old import ContSysOld
...
class LinSysOld:
    class Option:
        @dataclass
        class Base(ContSysOld.Option.Base):
    ...
    class Entity(ContSysOld.Entity):
        def __init__(
            self,
            xa,
            ub: np.ndarray = None,
        ...
        def __exponential(self, option: LinSysOld.Option.Euclidean):
            xa_abs = abs(self._xa)
            xa_power = [self._xa]
            xa_power_abs = [xa_abs]
        ...
        def __compute_time_interval_err(self, option: LinSysOld.Option.Euclidean):
            # initialize asum
        ...
        def __input_tie(self, option: LinSysOld.Option.Euclidean):
            # initialize asum
            asum_pos = np.zeros((self.dim, self.dim), dtype=float)
            asum_neg = np.zeros((self.dim, self.dim), dtype=float)
        ...
        def __input_solution(self, option: LinSysOld.Option.Euclidean):
        ...
        def __reach_init_euclidean(
            self, r: Geometry.Base, option: LinSysOld.Option.Euclidean
        ):
        ...
        def delta_reach(self, r: Geometry.Base, option: LinSysOld.Option.Euclidean):
            rhom_tp_delta = (
        ...
        def error_solution(
            self, option: LinSysOld.Option.Euclidean, v_dyn: Geometry.Base, v_stat=None
        ):

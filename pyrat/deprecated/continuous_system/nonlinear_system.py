...
class NonLinSys:
    ...
    class Option:
        ...
        class __Base(Option.Base):
            algorithm: Option.ALGORITHM = Option.ALGORITHM.INVALID

            @abstractmethod
            def __post_init__(self):
                raise Exception("Implicit option, use explicit option instead.")

        @dataclass
        class Linear(__Base):
            algorithm = Option.ALGORITHM.LINEAR

            def __post_init(self):
                raise NotImplementedError

        @dataclass
        class Polynomial(ABC, __Base):
            algorithm: Option.ALGORITHM = Option.ALGORITHM.POLYNOMIAL
            raise NotImplementedError

        def cvt2(self, source: __Base, target: Option.ALGORITHM):
            def __linear2nonlinear():
                raise NotImplementedError

            def __nonlinear2linear():
                raise NotImplementedError

            if isinstance(source, NonLinSys.Option):
                raise NotImplementedError
